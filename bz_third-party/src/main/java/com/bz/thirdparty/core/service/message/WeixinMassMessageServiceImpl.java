 ...
 */
@Service(version="1.0.0",interfaceClass=com.bz.open.core.service.weixin.WeixinMessageService.class,protocol="hessian")
@Transactional
  ...
  private MessageRecordMapper messageRecordMapper;
	...
	public boolean sendTextMessage(WeixinMassMessageRequest message) throws ExternalException {
		...
		if(message.isSendToOne()){
			if(StringUtils.isEmpty(message.getWeixinUserOpenId().toString())) {
		    ...
		    logger.info(massMessage.getToUsers().toString());
		}else {
	...
	public boolean sendVideoMessage(WeixinMassMessageRequest message,InputStream inputStream) throws ExternalException {
	   ...
	   if(null==inputStream) {
		   logger.error("发送微信群发消息【视频格式】错误原因:{传入参数【inputstream】为空}");
			throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "传入参数【inputstream】为空");
	   }
	   //组装视频文件
	   WxMediaUploadResult uploadMediaRes=null;
	   try {
		 uploadMediaRes = wxMpService.getMaterialService().mediaUpload(WxConsts.MEDIA_VIDEO,message.getFileType(), inputStream);
	} catch (WxErrorException e) {
		logger.error("组装视频文件到微信服务器失败", e);
		throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "组装视频文件到微信服务器失败请检查参数【inputstream】");
	}
	...
	public boolean sendImageMessage(WeixinMassMessageRequest message,InputStream inputStream) throws ExternalException {
		logger.info("发送消息【图片消息】");
		 if(null==inputStream) {
		    logger.error("发送微信群发消息【图片格式】错误原因:{传入参数【inputstream】为空}");
		 ...
		 WxMediaUploadResult uploadMediaRes = null;
		try {
			uploadMediaRes = wxMpService.getMaterialService().mediaUpload(WxConsts.MEDIA_IMAGE, message.getFileType(),inputStream);
		} catch (WxErrorException e1) {
			logger.error("发送图片消息时上传图片文件到微信服务器失败", e1);
			e1.printStackTrace();
			throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送图片消息时上传图片文件到微信服务器失败");
		}
		 WxMpMassOpenIdsMessage massMessage = new WxMpMassOpenIdsMessage();
	...
	public boolean sendImageTextMessage(WeixinMassMessageRequest message,InputStream inputStream) throws ExternalException {
		logger.info("发送消息【图文消息】");
		if(null==inputStream) {
		   logger.error("发送微信群发消息【图文格式】错误原因:{传入参数【inputstream】为空}");
			throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送消息实体【WeixinMessage】属性【inputstream】为空");
	    }
		WxMediaUploadResult uploadMediaRes = null;
		WxMpMassUploadResult uploadImgMediaRes = null;
		try {
			uploadMediaRes = wxMpService.getMaterialService().mediaUpload(WxConsts.MEDIA_IMAGE, message.getFileType(),inputStream);
			WxMpMassNews news = new WxMpMassNews();
			WxMpMassNewsArticle wxmass = new WxMpMassNewsArticle();
			wxmass.setContent(message.getCount());
			wxmass.setThumbMediaId(uploadMediaRes.getMediaId());
			wxmass.setTitle(message.getTilte());
			news.addArticle(wxmass);
			message.setNews(news);
			uploadImgMediaRes = new WxMpMassMessageServiceImpl(wxMpService).massNewsUpload(message.getNews());
			logger.info("uploadImgMediaRes="+uploadImgMediaRes.getMediaId());
		} catch (WxErrorException e1) {
			logger.error("发送图文消息时上传图文文件到微信服务器失败", e1);
			e1.printStackTrace();
			throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送图文消息时上传图文文件到微信服务器失败");
		}
		 WxMpMassOpenIdsMessage massMessage = new WxMpMassOpenIdsMessage();
		 massMessage.setMsgType(WxConsts.MASS_MSG_NEWS);
		 massMessage.setMediaId(uploadImgMediaRes.getMediaId());
		  //组装接收人
		   if(message.isSendToOne()){
				if(StringUtils.isEmpty(message.getWeixinUserOpenId())) {
					logger.error("发送微信群发消息【图文格式】错误原因:{1:发送消息实体【WeixinMessage】属性->isSendToOne为true时属性【WeixinUserOpenId】为空}");
					throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送消息实体【WeixinMessage】属性【isSendToOne】为true时属性【WeixinUserOpenId】为空");
				}
				massMessage.getToUsers().add(message.getWeixinUserOpenId());
				massMessage.getToUsers().add("");
				...
				if(ListUtil.isEmpty(message.getWeixinUserOpenIds())){
					logger.error("发送微信群发消息【图文格式】错误原因:{1:发送消息实体【WeixinMessage】属性->isSendToOne为false时属性【WeixinUserOpenIds】为空}");
				     throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送消息实体【WeixinMessage】属性【isSendToOne】为false时属性【WeixinUserOpenIds】为空");
				}else {
					for(String opendid:message.getWeixinUserOpenIds()) {
						massMessage.getToUsers().add(opendid);
				...
				try {
					massResult = wxMpService.getMassMessageService().massOpenIdsMessageSend(massMessage);
					logger.info("发送图文消息发送返回结果:"+massResult);
					if(StringUtils.isEmpty(massResult.getErrorMsg())) {
						return false;
					}
					...
					logger.error("发送微信群发消息【图文消息错误】请求微信服务器报错,错误原因:【"+massResult.getErrorMsg()+"】",e);
					throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR,massResult.getErrorMsg());
				}
		return true;
	}
	...
	public boolean sendVoiceMessage(WeixinMassMessageRequest message,InputStream inputStream) throws ExternalException {
		logger.info("发送消息【语音消息】");
		 if(null==inputStream) {
			   logger.error("发送微信群发消息【mp3格式】错误原因:{传入参数【inputstream】为空}");
				throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "传入参数【inputstream】为空");
		   }
		 WxMediaUploadResult uploadMediaRes = null;
		try {
			uploadMediaRes = wxMpService.getMaterialService().mediaUpload(WxConsts.MEDIA_VOICE, message.getFileType(),inputStream);
		} catch (WxErrorException e1) {
		 ...
		 massMessage.setMediaId(uploadMediaRes.getMediaId());
		  //组装接收人
		   if(message.isSendToOne()){
				if(StringUtils.isEmpty(message.getWeixinUserOpenId())) {
					logger.error("发送微信群发消息【"+message.getFileType()+"格式】错误原因:{1:发送消息实体【WeixinMessage】属性->isSendToOne为true时属性【WeixinUserOpenId】为空}");
					throw new ExternalException(ExternalExceptionEnum.EXTERNAL_SEND_WEIXI_MASS_NMESSAGE_ERROR, "发送消息实体【WeixinMessage】属性【isSendToOne】为true时属性【WeixinUserOpenId】为空");
				}
				massMessage.getToUsers().add(message.getWeixinUserOpenId());
				massMessage.getToUsers().add("");
	 ...
	 * 描述:将发送微信消息记录存入数据库中
	 * 备注:
	 * @param message:消息实体封装{@link WeixinMassMessageRequest}
	 * @throws WxErrorException
	 */
	private void addSendMessageRecord(WeixinMassMessageRequest message)throws WxErrorException{
		if(message.isRecord()) {
